...
const ParentProfileForm = (props) => {
  ...
  const birthDateHandler = (data) => {
    ...
    const validDate =
      newDate === 'dd/mm/yyyy' ? '' : momentDateToStr(data.toString());
    const newData = {
      ...formData,
      profile: { ...formData.profile, date_of_birth: validDate },
    };
    setFormData(newData);
  };
  const phoneInputChangeHandler = (number) => {
    const newData = {
      ...formData,
      profile: { ...formData.profile, phone_number: number },
    };
    setFormData(newData);
  };
  ...
  return (
    <>
      ...
      {countriesOptions && citiesOptions && (
        <div className="form__row form__row_flex">
          ...
          {countriesOptions.find(
            (option) => option.value === formData.profile.country
          )?.label === 'Россия' && (
            <div className="form__col2 form__label">
              <FormField
      ...
      <div className="form__row form__row_flex">
        <div className="form__col2 is-hidden">
          <label htmlFor="passw" className="form__label">
            Пароль
          </label>
          {!passwordChanging && (
            <div className="form__visible-block">
